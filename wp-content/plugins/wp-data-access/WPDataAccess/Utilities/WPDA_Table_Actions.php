	...
	class WPDA_Table_Actions {
		...
		public function show() {
			if ( ! isset( $_REQUEST['table_name'] ) || ! isset( $_REQUEST['schema_name'] ) ) {
				...
				?>
				<div id="<?php echo esc_attr( $this->table_name ); ?>-tabs">
					<div class="nav-tab-wrapper" style="padding-top: 0 !important;">
						<?php
						echo '<a id="' . esc_attr( $this->table_name ) . '-sel-1" class="nav-tab nav-tab-active wpda-manage-nav-tab' .
						...
						if ( 'Table' === $this->dbo_type || 'View' === $this->dbo_type ) {
							echo '<a id="' . esc_attr( $this->table_name ) . '-sel-6" class="nav-tab wpda-manage-nav-tab' .
						...
						if ( '' !== $this->dbo_type ) {
							echo '<a id="' . esc_attr( $this->table_name ) . '-sel-2" class="nav-tab wpda-manage-nav-tab' .
						...
						if ( 'Table' === $this->dbo_type ) {
							echo '<a id="' . esc_attr( $this->table_name ) . '-sel-3" class="nav-tab wpda-manage-nav-tab' .
						...
						if ( 'Table' === $this->dbo_type ) {
							echo '<a id="' . esc_attr( $this->table_name ) . '-sel-5" class="nav-tab wpda-manage-nav-tab' .
						...
						if ( 'Table' === $this->dbo_type || 'View' === $this->dbo_type ) {
							echo '<a id="' . esc_attr( $this->table_name ) . '-sel-4" class="nav-tab wpda-manage-nav-tab' .
								...
								<li>
									...
									<a href="https://wpdataaccess.com/docs/documentation/data-explorer/table-settings/" target="_blank">
										<span class="dashicons dashicons-editor-help"
											  title="<?php echo sprintf( __( 'Add dynamic hyperlinks to table `%s` [help opens in a new tab or window]', 'wp-data-access' ), $this->table_name ); ?>"
...
Variable $$column_name$$ will be replaced with the value of column $$column_name$$ in table `' . $this->table_name . '`.', 'wp-data-access' ); ?>"
														  style="cursor:pointer;"></span>
												</th>
												<th></th>
											</tr>
											...
											if ( isset( $settings_db_custom->hyperlinks ) && is_array( $settings_db_custom->hyperlinks ) ) {
												foreach ( $settings_db_custom->hyperlinks as $hyperlink ) {
													...
													?>
													<tr id="wpda_<?php echo $this->table_name; ?>_add_hyperlink_<?php echo $no_hyperlinks; ?>">
														<td style="padding-left:0;vertical-align:top;">
															<input id="hyperlink_label_<?php echo $no_hyperlinks; ?>"
																   class="wpda_action_font"
																   type="text"
																   value="<?php echo esc_attr( $hyperlink_label ); ?>"
															/>
															<input id="hyperlink_label_<?php echo $no_hyperlinks; ?>_old"
																   type="hidden"
																   value="<?php echo esc_attr( $hyperlink_label ); ?>"
															/>
														</td>
														<td style="vertical-align:top;text-align:center;">
															<input id="hyperlink_list_<?php echo $no_hyperlinks; ?>"
																   class="wpda_action_font"
																   type="checkbox"
														...
														<td style="vertical-align:top;text-align:center;">
															<input id="hyperlink_form_<?php echo $no_hyperlinks; ?>"
																   class="wpda_action_font"
																   type="checkbox"
																   style="margin-top:7px;"
														...
														<td style="vertical-align:top;text-align:center;">
															<input id="hyperlink_target_<?php echo $no_hyperlinks; ?>"
																   class="wpda_action_font"
																   type="checkbox"
																   style="margin-top:7px;"
														...
														<td style="vertical-align:top;">
															<textarea id="hyperlink_html_<?php echo $no_hyperlinks; ?>"
																	  rows="5"
																	  class="wpda_action_font"
																	  style="min-width:300px;resize:both;"
															><?php echo urldecode( $hyperlink_html ); ?></textarea>
															<input id="hyperlink_html_<?php echo $no_hyperlinks; ?>_old"
																   type="hidden"
														...
														<td style="vertical-align:top;">
															<a href="javascript:void(0)"
															   ...
															   onclick="if (confirm('<?php echo sprintf( __( 'Delete hyperlink %s?', 'wp-data-access' ), esc_attr( $hyperlink_label ) ); ?>')) { jQuery('#wpda_<?php echo $this->table_name; ?>_add_hyperlink_<?php echo $no_hyperlinks; ?>').remove(); }"
															></a>
														</td>
													</tr>
													<?php
											...
											?>
											<tr id="wpda_<?php echo $this->table_name; ?>_add_hyperlink_link"
											...
											>
												<td colspan="3" style="padding-left:0;">
													<a class="wpda_table_settings_add_menu" href="javascript:void(0);"
													   onclick="jQuery('#wpda_<?php echo $this->table_name; ?>_add_hyperlink_link').hide(); jQuery('#wpda_<?php echo $this->table_name; ?>_add_hyperlink').show();">
														&raquo; Add Hyperlink
													</a>
												</td>
											</tr>
											<tr id="wpda_<?php echo $this->table_name; ?>_add_hyperlink"
												<?php if ( 0 !== $no_hyperlinks ) {
										...
										<div>
											<input type="button"
												   id="wpda_<?php echo $this->table_name; ?>_save_hyperlinks"
												   value="<?php echo __( 'Save Dynamic Hyperlinks', 'wp-data-access' ); ?>"
												   class="button button-primary"
												   style="font-size: 90%;"
											/>
											<input type="button"
												   id="wpda_<?php echo $this->table_name; ?>_cancel_hyperlinks"
												   value="<?php echo __( 'Cancel', 'wp-data-access' ); ?>"
												   class="button button-primary"
												   style="font-size: 90%;"
											/>
											<input id="no_hyperlink_<?php echo $this->table_name; ?>"
								...
								<li>
									...
									<ul class="wpda_table_settings_nested">
										...
										<div>
											<input type="button"
											       id="wpda_<?php echo $this->table_name; ?>_save_column_settings"
											       value="<?php echo __( 'Save Column Settings', 'wp-data-access' ); ?>"
											       class="button button-primary"
											       style="font-size: 90%;"
											/>
											<input type="button"
											       id="wpda_<?php echo $this->table_name; ?>_cancel_column_settings"
											       value="<?php echo __( 'Cancel', 'wp-data-access' ); ?>"
											       class="button button-primary"
											       style="font-size: 90%;"
											/>
								...
								<li>
									...
									<ul class="wpda_table_settings_nested">
										<table class="wpda_table_settings">
											...
											?>
											<tr id="wpda_<?php echo $this->table_name; ?>_add_menu_link"
											<?php if ( 0 === $no_menus ) {
												echo 'style="display:none;background:inherit;"';
											} else {
												echo 'style="background:inherit;"';
											...
											>
												<td colspan="3" style="padding-left:0;">
													<a class="wpda_table_settings_add_menu" href="javascript:void(0);"
													   onclick="jQuery('#wpda_<?php echo $this->table_name; ?>_add_menu_link').hide(); jQuery('#wpda_<?php echo $this->table_name; ?>_add_menu').show();">
														&raquo; Add menu
													</a>
												</td>
											</tr>
											<tr id="wpda_<?php echo $this->table_name; ?>_add_menu"
											<?php if ( 0 !== $no_menus ) {
												echo 'style="display:none;"';
											} ?>
											>
										...
										<div>
											<input type="button"
											       id="wpda_<?php echo $this->table_name; ?>_save_dashboard_menus"
											       value="<?php echo __( 'Save Dashboard Menus', 'wp-data-access' ); ?>"
											       class="button button-primary"
											       style="font-size: 90%;"
											/>
											<input type="button"
											       id="wpda_<?php echo $this->table_name; ?>_cancel_dashboard_menus"
											       value="<?php echo __( 'Cancel', 'wp-data-access' ); ?>"
											       class="button button-primary"
											       style="font-size: 90%;"
											/>
							...
							<input class="wpda_table_setting_item" type="hidden" id="wpda_<?php echo $this->table_name; ?>_sql_dml"
			...
			<script type='text/javascript'>
				jQuery(document).ready(function () {
					...
					// Dynamic hyperlinks
					jQuery('#wpda_<?php echo $this->table_name; ?>_save_hyperlinks').click( function() {
						return submit_hyperlinks('<?php echo $this->table_name; ?>');
					});
					jQuery('#wpda_<?php echo $this->table_name; ?>_cancel_hyperlinks').click( function() {
						jQuery('#wpda_admin_menu_actions_<?php echo $this->table_name; ?>').toggle();
						wpda_toggle_row_actions('<?php echo $this->table_name; ?>', '<?php echo $_REQUEST['rownum']; ?>');
					});

					// Column settings
					jQuery('#wpda_<?php echo $this->table_name; ?>_save_column_settings').click( function() {
						return submit_column_settings('<?php echo $this->table_name; ?>');
					});
					jQuery('#wpda_<?php echo $this->table_name; ?>_cancel_column_settings').click( function() {
						jQuery('#wpda_admin_menu_actions_<?php echo $this->table_name; ?>').toggle();
						wpda_toggle_row_actions('<?php echo $this->table_name; ?>', '<?php echo $_REQUEST['rownum']; ?>');
					});

					// Dashboard menus
					jQuery('#wpda_<?php echo $this->table_name; ?>_save_dashboard_menus').click( function() {
						return submit_dashboard_menus('<?php echo $this->table_name; ?>');
					});
					jQuery('#wpda_<?php echo $this->table_name; ?>_cancel_dashboard_menus').click( function() {
						jQuery('#wpda_admin_menu_actions_<?php echo $this->table_name; ?>').toggle();
						wpda_toggle_row_actions('<?php echo $this->table_name; ?>', '<?php echo $_REQUEST['rownum']; ?>');
					});
				});
				...
				function submit_hyperlinks(table_name) {
					hyperlinks = [];
					unused = {};
					isvalid = true;

					no_hyperlink = jQuery('#no_hyperlink_<?php echo $this->table_name; ?>').val();
					if (isNaN(no_hyperlink)) {
						return false;
					}
					
					for (hyperlink_no=0; hyperlink_no<=no_hyperlink;hyperlink_no++) {
						if (jQuery('#hyperlink_label_' + hyperlink_no).length) {
							hyperlink_label = jQuery('#hyperlink_label_' + hyperlink_no).val();
							hyperlink_html = jQuery('#hyperlink_html_' + hyperlink_no).val();

							hyperlink_label_old = jQuery('#hyperlink_label_' + hyperlink_no + '_old').val();
							hyperlink_html_old = jQuery('#hyperlink_html_' + hyperlink_no + '_old').val();

							if (hyperlink_label != '' && hyperlink_html != '') {
								hyperlink_list = jQuery('#hyperlink_list_' + hyperlink_no).is(':checked');
								hyperlink_form = jQuery('#hyperlink_form_' + hyperlink_no).is(':checked');
								hyperlink_target = jQuery('#hyperlink_target_' + hyperlink_no).is(':checked');
								...
								if (hyperlink_label_old != '' || hyperlink_html_old != '') {
									alert('<?php echo __( 'Hyperlink label and HTML must be entered', 'wp-data-access' ); ?>');
									isvalid = false;
								}
							}
						}
					...
					unused["sql_dml"] = jQuery('#wpda_<?php echo $this->table_name; ?>_sql_dml').val();
				...
				function submit_column_settings(table_name) {
					custom_settings = {};
					list_labels = {};
					form_labels = {};
					column_media = {};
				...
				function submit_dashboard_menus(table_name) {
					menu = {};
					isvalid = true;
					...
					jQuery('#wpda_table_settings_' + table_name + ' .wpda_table_setting_item_menu').each(function () {
						...
						if ('' !== menu_name && '' !== menu_slug) {
							...
							if (menu_id!=='') {
								alert('<?php echo __( 'Menu name and menu slug must be entered', 'wp-data-access' ); ?>');
								isvalid = false;
							}
						}
					});
