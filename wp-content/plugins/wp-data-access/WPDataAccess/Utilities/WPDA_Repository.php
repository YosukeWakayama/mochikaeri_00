		...
		protected function cleanup() {
			...
			if ( WPDA_Publisher_Model::table_exists() ) {
				...
				$alter = "alter table " . WPDA_Publisher_Model::get_base_table_name() .
						 ...
						 " modify pub_responsive_cols int unsigned";
				$wpdb->query( $alter );
			}
			...
			if ( WPDA_Table_Settings_Model::table_exists() ) {
				$settings = $wpdb->get_results(
					'select wpda_schema_name, wpda_table_name from ' . WPDA_Table_Settings_Model::get_base_table_name() .
					" where wpda_schema_name = ''",
					'ARRAY_A'
				);
				foreach ( $settings as $setting ) {
					$wpdb->query(
						'update ' . WPDA_Table_Settings_Model::get_base_table_name() . " " .
						"set wpda_schema_name = '{$wpdb->dbname}' " .
						"where wpda_schema_name = '' " .
						"and wpda_table_name = '{$setting['wpda_table_name']}'"
					);
				}
			}
