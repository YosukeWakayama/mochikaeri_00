...
class WPInventoryInit extends WPIMCore {
	...
	private static function add_actions() {
		...
		add_action( 'upgrader_process_complete', [ __CLASS__, 'upgrader_process_complete' ], 10, 2 );
...
STYLE;
	...
	public static function register_common_scripts() {
		wp_register_script( 'wpinventory-stepper', self::$url . 'js/jquery.stepper.js' );
	...
	private static function plugin_updater() {
		...
		foreach ( $add_ons AS $add_on ) {
			if ( ! empty( $add_on->lkey ) && ! empty( $add_on->version ) ) {
				// setup the updater
				new WPIMUpdater( WPIMAPI::API_URL, $add_on->path, [
